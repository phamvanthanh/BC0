<template>
<div  class="wrapper">
    <HotTable :root="root" 
                :settings="hotSettings"
                :data="leaves"                               
                 >
    </HotTable>

     <modal v-if="showModal" 
            :id="auditId"
            :fileType="fileType"
            :fileFormat="fileFormat"
            @close="showModal = false"
            :uploadUrl="uploadUrl">
    <!--
      you can use custom content here to overwrite
      default content
        -->
        <h3 slot="header">custom header</h3>
    </modal>
</div>

</template>
<script>
import moment from 'moment';
import numbro from 'numbro';
import pikaday from 'pikaday';
import Zeroclipboard from 'zeroclipboard';

import HotTable from 'vue-handsontable-official';
import modal from './uploadModal';

    
    export default {
        data() {
              
            return {
                auditId: null,
                jid: this.$route.params.jid,
                showModal: false,
                leaves:  null,
                root: 'unitify',
                hotSettings: {
                  startCols: 7,           
                  rowHeaders: true,
                  allowInsertRow: true,
               
                  rowHeaders: true,
                  copyPaste: true,
                  copyable: true,
                  columns: [
                         
                          {data: 'code', readOnly: true},
                          {data: 'name', readOnly: true},
                          {data: 'quantity', readOnly: true},
                          {data: 'unit', readOnly: true },
                          {data: 'file',  readOnly: true},
                          {data: 'markdown', readOnly: true},
                          {data: 'commit', type: 'checkbox', readOnly: true},
                          {data: 'a_quantity'},                        
                          {data: 'a_file'  },
                          {data: 'a_markdown'},
                          {data: 'a_commit', type: 'checkbox'},

                  ],
                  colHeaders: ['Code', 'Name', 'Q', 'U', 'File', 'Mrk', 'C', 'AQ',  'AFile', 'AMrk', 'AC'],
                  stretchH: 'all',
                  afterChange: function(change, source) {                      
                      if(source == 'edit') {
                          ;
                         

                          var change = change[0], data;   
                          if(change[1] == 'a_quantity' || change[1] == 'a_commit'){
                              var row  = this.getSourceDataAtRow(change[0]);
                            
                               if(change[2] != change[3]) {
                                    
                                    data = {                   
                                    
                                        quantity_id      : row.quantity_id==null? row.id: row.quantity_id,
                                        quantity  : row.a_quantity,
                                        commit    : row.a_commit
                                    }
                                    axios.post('/jobs/'+jid+'/audit', data)
                                            .then(({data}))
                                            .catch((error)=>console.log(error))                              
                                }

                          }          
                          
                      
                         
                        
                      }                      
                  },
                  afterBeginEditing: function(row, col) {
                      if(col == 8 || col == 9) {
                           var rowData  = this.getSourceDataAtRow(row); 
                           console.log(rowData);
                           if(rowData.a_quantity == null) {
                               alert('You must fill quantity before uploading file');
                               return;
                           }   
                           let type =  col == 8? 'quantity': 'markdown'; 
                           let format  = type =='quantity'? 'application/.xls,.xlsx' : 'image/*,application/pdf';
                           let uploadUrl = '/jobs/'+jid+'/audit/files';  
                           let payload = {format: format, type: type, audit_id: rowData.audit_id, uploadUrl: uploadUrl};
                           bus.$emit('showUploadModal', payload);      

                      }                 
                  },
                }
            }          
        
        },
        created() {
            let _this = this;
            this.getData();
           
            window.jid = this.$route.params.jid;
            
            bus.$on('showUploadModal', function(payload) {                
                _this.auditId = payload.audit_id.toString();
                _this.fileType = payload.type;
                _this.fileFormat = payload.format;
                _this.showModal = true;
                _this.uploadUrl = payload.uploadUrl;
            });
             bus.$on('refreshtable', function() {
                _this.getData();
                _this.showModal = false;
            })

        },       
            
        computed: {
            notifications() {
                return this.$store.state.notifications
            },            
        },
       
        components: {    
         
            HotTable,
            modal        
           
                      
        },
        methods: {
            getData() {
                 axios.get('/jobs/'+this.jid+'/audit')
                 .then(({data})=>{        
                   this.leaves = data;})
                 .catch((error)=>{
                   console.log(error)});
               
            }

           
        }
      
    }
</script>


